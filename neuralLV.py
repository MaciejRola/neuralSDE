...
class NeuralLV(nn.Module):
    ...
    def forward(self, options, test=False):  # torch version
        ...
        for i in range(1, self.N_steps + 1):
            ...
            drift = S_prev * r / (1 + S_prev.detach() * r * torch.sqrt(dt))
            diffusion = leverage / (1 + leverage.detach() * torch.sqrt(dt))

            if test:
                NN = self.test_normal_variables[:, i - 1]
            else:
    ...
    for epoch in range(epochs):
        if model.use_hedging:
            ...
            for batch in range(0, 20 * batch_size, batch_size):
                optimizer_SDE.zero_grad()
                init_time = time.time()
                prices, _ = model(options, batch_size)
                time_forward = time.time() - init_time
                itercount += 1
                loss = loss_fn(prices, target_prices)
                init_time = time.time()
