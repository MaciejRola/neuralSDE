...
def train(model, maturities, strikes, target, batch_size, epochs, threshold=2e-5):
    ...
    for epoch in range(epochs):
        ...
        with open(f"Results/log_eval_NeuralLV_use_hedging_{model.use_hedging}.txt", "a") as f:
        ...
        if model.use_hedging:
            ...
            if len(LOSSES) > 1 and len(VARIANCES) > 1:
                ...
                axs[1].legend()
                plt.savefig(f'Results/loss_and_var_NeuralLV_use_hedging_{model.use_hedging}.png')
                plt.close()
        else:
            if len(LOSSES) > 1:
                plt.plot(LOSSES, label='RMSE Error')
                plt.legend()
                plt.savefig(f'Results/loss_neuralLV_use_hedging_{model.use_hedging}.png')
                plt.close()
        ...
        if loss_val < loss_val_best:
            ...
            filename = f'Results/NeuralLV_use_hedging_{model.use_hedging}.pth.tar'
