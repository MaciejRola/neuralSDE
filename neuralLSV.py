...
def train(model, maturities, strikes, target, batch_size, epochs, threshold=2e-5):
    ...
    for epoch in range(epochs):
        ...
        with open("log_eval_LSV.txt", "a") as f:
...
if __name__ == '__main__':
    ...
    target = torch.tensor(pd.pivot_table(options, values='Price', index='Strike', columns='Expiration_date').to_numpy(), requires_grad=True, dtype=torch.float) / 100
